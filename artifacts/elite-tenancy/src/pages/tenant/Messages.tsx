...
import { Send, MessageCircle, Wifi, WifiOff, ArrowLeft } from "lucide-react";
...
function ConversationItem({
  ...
  const sentByMe = conv.lastMessage?.senderId === myUserId;
...
export default function TenantMessages() {
  ...
  // Resolve my DB user id from the first conversation (we don't have direct access to DB userId here)
  // We compare senderId to determine "isMe" using the conversation context
  const myDbUserId = useRef<number | undefined>(undefined);
  ...
  // Determine my userId: I'm whichever participant is NOT "other"
  // We infer it from message senderId — first message sent by someone != other.id
        ...
        >
          {activeConv ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
              ...
              <h2 className="font-semibold text-foreground mb-2">Your Messages</h2>
              <p className="text-sm text-muted-foreground max-w-xs">
                Select a conversation from the left to read and reply to messages from your landlord.
              </p>
            </div>
          )}
        </div>
